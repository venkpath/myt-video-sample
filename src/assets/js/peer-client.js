peerapp = (function() {
    ...
    function connect(c) {
        ...
        if (c.label === 'chat') {
            ...
            c.on('data', function(data) {
                // myapp.appendAttachmentHistory(c.peer,data)
                // If we're getting a file, create a URL for it.
                if (data.constructor === ArrayBuffer) {
                    var dataView = new Uint8Array(data);
                    var dataBlob = new Blob([dataView]);
                    var file = new File([dataBlob], 'filename.jpg', {type: "image/jpeg", lastModified: Date.now()});

                    // var url = window.URL.createObjectURL(dataBlob);
                    // $('#' + c.peer).find('.messages').append('<div><span class="file">' +
                    //     c.peer + ' has sent you a <a target="_blank" href="' + url + '">file</a>.</span></div>');
                        myapp.appendAttachmentHistory(c.peer,file);
                }
    ...
    function peerCallbacks(peer) {
        ...
        peer.on('call', function(call) {
            ...
            // } else {
                window.incomingCall = call
                if(screenShareVal=='false'){

                myapp.showIncomingCall(call.peer, call.options.metadata);
                }else{
                    myapp.showScreenShareIncomingCall(call.peer, call.options.metadata);
                    screenShareVal = 'false';
                }
            // }
            // }
        });
    ...
    function sendFile(peerId, file) {
        ...
        if(connectedPeers[peerId+'-file']) {
            var conn = connectedPeers[peerId+'-file']
            conn.send(file)
        }
    }
    ...
    function screenShare(callerID) {
        var options = {audio: true, video: true};
          
        initializeScreenShare(options,function(){
            myapp.showVideoCall(options);      
    ...
    function receiveShareConnect(call) {
        
        var call = window.incomingCall;
        var metadata = call.options.metadata;
        console.log(metadata);

        initializeScreenShare(metadata, function() {
            call.answer(window.localStream);
            myapp.showVideoCall(metadata);
            callConnect(call)
        });    
    ...
    function initializeScreenShare(options, callback) {
    ...
    return {
        ...
        receiveShareConnect:receiveShareConnect,
        sendFile:sendFile
    }
})();
